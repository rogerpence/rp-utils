...
describe("formatDateToYYYYMMDD", () => {
    ...
    //     expect(convertDateToStringYYYY_MM_DD(morning)).toBe("2025-06-15");
    //     expect(convertDateToStringYYYY_MM_DD(evening)).toBe("2025-06-15");
    // });

    it("should handle dates created from timestamp", () => {
        const date = new Date(1700000000000); // Nov 14, 2023
        expect(convertDateToStringYYYY_MM_DD(date)).toMatch(
            /^\d{4}-\d{2}-\d{2}$/
        );
    });
});
